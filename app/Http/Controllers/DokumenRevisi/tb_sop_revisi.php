...
class tb_sop_revisi extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.Menus.DokumenRevisi.SOP.datasopRevisi', compact('sopRevisi'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('dataSopRevisi.index')->with('success', 'Pengajuan revisi dan Draft PDF berhasil dibuat.');
        } catch (\Exception $e) {
            return redirect()->route('dataSopRevisi.index')->with('error', 'Data tersimpan, tapi PDF gagal dibuat: ' . $e->getMessage());
        }
    }

    public function approvePJO(Request $request)
    {
        $doc = sop_revisi::findOrFail($request->id);
        ...
        if ($action == 'approve') {
            try {
                ...
                $doc->PJOApprove = 'approved';
                $doc->PJOFeedback = $request->feedback;
                ...
                //sampai sini bisa jalan, teptai lebih dari $newFileName sudah tidak bisa halan
                $newFileName = $this->generateRevisiPDF($doc, $lampiranData);

                $doc->update(['file' => $newFileName]);
                $tabel = 'tb_sop_' . strtolower(str_replace(' ', '_', trim($doc->departemen))) . 's';
                ...
                \DB::table($tabel)->insert([
                    ...
                    'DHdanSH'                  => $doc->DHdanSH,
                    'DHdanSHApprove'           => $doc->DHdanSHApprove,
                ...
                if (Storage::exists('public/sop_revisi_pending/' . $newFileName)) {
                    Storage::move('public/sop_revisi_pending/' . $newFileName, 'public/sop/' . $newFileName);
                }

                $doc->delete();

                \DB::commit();
                return redirect()->route('dataSopRevisi.index')->with('success', 'Revisi berhasil diterbitkan dengan pengesahan lengkap.');
            ...
            return back()->with('success', 'Revisi ditolak.');
        }
    }
    ...
    public function approveDHSH(Request $request)
    {
        ...
        if ($action == 'approve') {
            ...
            if ($doc->file && Storage::exists('public/sop_revisi_pending/' . $doc->file)) {
                Storage::delete('public/sop_revisi_pending/' . $doc->file);
            }
    ...
    private function generateRevisiPDF($revisi, $lampiranData)
    {
        ...
        $tabel = 'tb_sop_' . strtolower(str_replace(' ', '_', trim($revisi->departemen))) . 's';
        ...
        Storage::put('public/sop_revisi_pending/' . $fileName, $pdf->output());
    ...
    {
        $sopRevisi = sop_revisi::findOrFail($id);
        if ($sopRevisi->file) {
            Storage::delete('public/sop_revisi_pending/' . $sopRevisi->file);
        }
        $sopRevisi->delete();
        return redirect()->route('dataSopRevisi.index')->with('success', 'Data revisi berhasil dihapus.');
    }
}