...
<div class="card">
  <div class="card-header d-flex justify-content-between align-items-center">
    <h5 class="card-header">Tabel Data SP PRODUKSI</h5>
    @if(Auth::user()->role == 0 || Auth::user()->role == 1)
    <a href="{{route('dataSpProduksi.inactive')}}" type="button" class="btn btn-primary" >
            Dokumen Tidak Aktif
        </a>
      ...
      @foreach ($sp as $item)
        <tr>
            ...
            <a href="javascript:void(0)" 
              onclick="handleRevision('{{ $item->no_dokumen }}', '{{ route('dataSpProduksi.edit', $item->id) }}')" 
              class="btn btn-sm btn-warning">
              Revisi
            </a>
            <!-- <a href="{{ route('dataSpProduksi.edit', $item->id) }}"class="btn btn-sm btn-secondary">Revisi</a> -->
            <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                action="{{ route('dataSpProduksi.destroy', $item->id) }}" method="POST">
                @csrf
                @method('DELETE')
