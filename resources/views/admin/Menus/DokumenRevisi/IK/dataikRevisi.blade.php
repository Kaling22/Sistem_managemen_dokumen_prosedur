...
<div class="card">
  ...
  <div class="table-responsive text-nowrap">
    <table class="table">
      ...
      @foreach ($ikRevisi as $index => $item)
        <tr>
            ...
            <td>
                @if($item->file)
                    <a href="{{ asset('storage/ik_revisi_pending/'.$item->file) }}" target="_blank">
                        Lihat PDF
                    </a>
                @else
            ...
            {{-- Kolom Feedback --}}
            <td>
                @if($item->DHdanSHFeedback)
                    <button type="button" class="btn btn-sm btn-outline-secondary" 
                            onclick="openFeedbackModal({{ json_encode($item->DHdanSHFeedback) }})">
            ...
            <td>
              <div class="d-flex gap-1">
                ...
                <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                    action="{{ route('dataIkRevisi.destroy', $item->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
...
<div class="modal fade" id="modalApproveDH" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        {{-- Route diarahkan ke revisi.ik.approve.dhsh --}}
        <form action="{{ route('revisi.ik.approve.dhsh') }}" method="POST">
            @csrf
            <input type="hidden" name="id" id="doc_id">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Approval Section/Dept Head (Revisi IK)</h5>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" name="action" value="approve" id="btn-approve-dh" class="btn btn-success">Approve & Publish</button>
                </div>
            </div>
        </form>
    </div>
...
<script>
    ...
    function openFeedbackModal(dhshFeedback) {
