...
<div class="card">
  ...
  <div class="table-responsive text-nowrap">
    <table class="table">
      ...
      @foreach ($spRevisi as $index => $item)
        <tr>
            ...
            <td>
                @if($item->file)
                    <a href="{{ asset('storage/sp_revisi_pending/'.$item->file) }}" target="_blank">
                        Lihat PDF
                    </a>
                @else
            ...
            <td>
                @if($item->DHdanSHFeedback)
                    <button type="button" class="btn btn-sm btn-outline-secondary" 
                            onclick="openFeedbackModal('{{ $item->DHdanSHFeedback ?? '-' }}'">
            ...
            <td>
              <div class="d-flex gap-1">
                ...
                <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                    action="{{ route('dataSpRevisi.destroy', $item->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
...
<div class="modal fade" id="modalApproveDH" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        {{-- Route diarahkan ke revisi.sp.approve.dhsh --}}
        <form action="{{ route('revisi.sp.approve.dhsh') }}" method="POST">
            @csrf
            <input type="hidden" name="id" id="doc_id">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Approval Section/Dept Head (Revisi SP)</h5>
