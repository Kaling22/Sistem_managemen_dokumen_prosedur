...
<div class="card">
  <div class="card-header d-flex justify-content-between align-items-center">
    ...
    @if(in_array(Auth::user()->role, [0, 1]))
      <a href="{{ route('dataIkBaru.create') }}" class="btn btn-primary">
          Tambah Dokumen Baru
      </a>
    @endif
  </div>
            ...
            <td>
                @if($item->file)
                    @php
                        ...
                        $folder = ($item->status_doc == 'Active') ? 'ik' : 'ik_pending';
                    @endphp
            ...
            <td>
              <div class="d-flex gap-1">
                ...
                <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                    action="{{ route('dataIkBaru.destroy', $item->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
...
<div class="modal fade" id="modalApproveDH" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <form action="{{ route('dokumen.approve.dhshik') }}" method="POST">
            @csrf
