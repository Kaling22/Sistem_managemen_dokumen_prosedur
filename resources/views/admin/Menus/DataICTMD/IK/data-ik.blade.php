...
<div class="card">
  <div class="card-header d-flex justify-content-between align-items-center">
    <h5 class="card-header">Tabel Data IK ICTMD</h5>
    @if(Auth::user()->role==0)
    <a href="{{route('dataIkIctmd.inactive')}}" type="button" class="btn btn-primary" >
            Dokumen Tidak Aktif
        </a>  
      ...
      @foreach ($ik as $item)
        <tr>
            ...
            <a href="javascript:void(0)" 
              onclick="handleRevision('{{ $item->no_dokumen }}', '{{ route('dataIkIctmd.edit', $item->id) }}')" 
              class="btn btn-sm btn-warning">
              Revisi
            </a>
            <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                action="{{ route('dataIkIctmd.destroy', $item->id) }}" method="POST">
                @csrf
                @method('DELETE')
